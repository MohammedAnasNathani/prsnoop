...
def stats_of(activity: Activity) -> Stats:
...
def render_table(activity: Activity) -> str:
    ...
    if s.momentum:
        ...
        lines.append(f"  Momentum           {s.momentum} ({pct} second half)")
    if s.busiest_day:
...
def render_markdown(activity: Activity) -> str:
    ...
    if trend:
        ...
        for d in trend:
            out.append(
                f"| {d.label} | {d.previous:g} | {d.current:g} | {_fmt_trend_value(d)} |"
            )
        out.append("")
    if s.repo_performance:
        out += [
...
.hm .h0 { fill: var(--line); }
.hm .h1 { fill: #9be9a8; } .hm .h2 { fill: #40c463; }
.hm .h3 { fill: #2ea043; } .hm .h4 { fill: #216e39; }
@media (prefers-color-scheme: dark) { .hm .h0 { fill: #161b22; }
  .hm .h1 { fill: #0e4429; } .hm .h2 { fill: #006d32; }
  .hm .h3 { fill: #26a641; } .hm .h4 { fill: #39d353; } }
"""
...
def _svg_heatmap(day_map: dict[str, int], start: str, end: str) -> str:
    """GitHub-style contribution calendar as inline SVG.

    One cell per day from ``start`` to ``end``, columns are weeks, rows
    are weekdays. Colors go by quartile of the peak day, so the shape of
    the year is visible at a glance.
    """
    ...
    # pad the first week so weekday rows line up (Sunday first)
    for _i in range((d0.weekday() + 1) % 7):
        week.append(("", -1))
    cursor = d0
    while cursor <= d1:
        week.append((cursor.isoformat(), day_map.get(cursor.isoformat(), 0)))
        if len(week) == 7:
            weeks.append(week)
            week = []
    ...
    height = 7 * (cell + gap) + 6
    width = len(weeks) * (cell + gap) + 6
    rects: list[str] = []
    for wi, wk in enumerate(weeks):
        for di, (day, total) in enumerate(wk):
            if not day:
                continue
            x = 3 + wi * (cell + gap)
            y = 3 + di * (cell + gap)
            rects.append(
                f'<rect class="h{level(total)}" x="{x}" y="{y}" '
                f'width="{cell}" height="{cell}" rx="2">'
                f"<title>{day}: {total} items</title></rect>"
            )
    return (
        f'<svg class="hm" viewBox="0 0 {width} {height}" role="img" '
        f'aria-label="contribution heatmap">{"".join(rects)}</svg>'
    )
...
def render_html(activity: Activity) -> str:
    ...
    heatmap_html = ""
    if s.since and s.day_activity and s.window_days > 45:
        day_map = {da.date: da.total for da in s.day_activity}
        heatmap = _svg_heatmap(day_map, s.since, s.last_activity_date or s.since)
        if heatmap:
            heatmap_html = (
                "<h2>Contribution calendar</h2>"
                "<p class=meta>each cell is one day, greener means more"
                " shipped</p>"
                f"{heatmap}"
            )
...
def render_org_html(prs: list[OrgPR], pulse: OrgPulse) -> str:
    ...
    day_activities = [DayActivity(date=d, prs=n) for d, n in s.day_counts]
    chart = _svg_chart(day_activities) if day_activities else ""
    chart_html = f"<h2>Opening activity</h2>{chart}" if chart else ""

    author_rows = "".join(
        f'<tr><td><a href="https://github.com/{esc(a)}">{esc(a)}</a></td><td>{n}</td></tr>'
        for a, n in s.top_authors
    )
    repo_rows = "".join(
        f'<tr><td><a href="https://github.com/{esc(r)}">{esc(r)}</a></td><td>{n}</td></tr>'
...
def render_org_markdown(prs: list[OrgPR], pulse: OrgPulse) -> str:
    ...
    if prs:
        ...
        for p in prs[:30]:
            out.append(
                f"| {p.state} | [{p.repo}#{p.number}]({p.url}) | {p.title} | {p.author} |"
            )
        out.append("")
    return "\n".join(out)


def render_profile_readme(activity: Activity) -> str:
    """Paste-ready GitHub profile README section with badges and stats."""
    s = stats_of(activity)
    lines = [
        f"<!-- generated by prsnoop; refresh with: prsnoop readme {activity.user} -->",
        "",
    ]
    lines.append(render_badge(activity).rstrip())
    lines += [
        "",
        "### GitHub activity, "
        + (s.since or f"last {s.window_days} days")
        + (f" to {s.until}" if s.until else ""),
        "",
        "| | |",
        "|---|---|",
        f"| Pull requests | {s.prs_authored} ({s.prs_merged} merged,"
        f" {s.merge_rate * 100:.0f}%) |",
        f"| Reviews given | {s.reviews_given} |",
        f"| Issues opened | {s.issues_opened} |",
        f"| Lines changed | +{s.lines_added} / -{s.lines_deleted} |",
        f"| Longest streak | {s.longest_streak_days} days |",
    ...
    if s.top_repos:
        top = " · ".join(
            f"[{repo}](https://github.com/{repo})" for repo, _n in s.top_repos[:5]
        )
        lines += ["", f"**Most active in:** {top}"]
    if s.languages:
        mix = " · ".join(f"{lang} {n}" for lang, n in s.languages[:5])
        lines += ["", f"**Languages:** {mix}"]
    lines += [
        "",
        "<sub>generated by"
        " [prsnoop](https://github.com/MohammedAnasNathani/prsnoop)"
        " | zero dependencies | MIT</sub>",
        "",
    ]
    return "\n".join(lines) + "\n"
