...
def _svg_heatmap(
    day_map: dict[str, int], start: str, end: str
) -> str:
...
def render_profile_readme(activity: Activity) -> str:
    ...
    s = stats_of(activity)
    lines = [
        "<!-- generated by prsnoop; refresh with:"
        f" prsnoop readme {activity.user} -->",
        "",
    ]
    lines.append(render_badge(activity).rstrip())
    lines += [
        "",
        "### GitHub activity, " + (s.since or f"last {s.window_days} days")
